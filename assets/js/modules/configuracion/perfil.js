      ...
      <div class="bg-white rounded-lg border border-slate-200 p-6">
        ...
        <div class="grid md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input class="input-base w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" name="dni" value="${perfil.dni || ''}" placeholder="12345678" maxlength="8" type="text" required />
          </div>
        ...
        <div class="grid md:grid-cols-2 gap-4">
          ...
          <div class="md:col-span-2 bg-slate-50 rounded p-3 border border-slate-200">
            <p class="text-xs text-slate-600">
              <strong>Ingreso al sistema:</strong> ${new Date(perfil.fechaIngreso || new Date()).toLocaleDateString('es-PE')}
            </p>
          </div>
        </div>
      </div>
...
export function initPerfilModule(mountNode) {
  const perfil = perfilService.obtener();
    ...
    <div class="max-w-4xl">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="md:col-span-1 flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg border border-blue-200 p-6 text-center">
          <div class="w-16 h-16 rounded-full bg-blue-300 flex items-center justify-center text-4xl mb-3">
            👤
          </div>
          <p class="font-semibold text-slate-900">${perfil.nombres} ${perfil.apellidos}</p>
          <p class="text-xs text-slate-600 mt-1">${perfil.cargo}</p>
          <p class="text-xs text-slate-500 mt-2 badge bg-blue-100 text-blue-800">${perfil.area}</p>
        </div>
